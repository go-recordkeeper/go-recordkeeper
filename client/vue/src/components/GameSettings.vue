...
const props = defineProps({
  ...
  create: Function as PropType<
    (request: CreateRecordRequest) => Promise<APIResponse<Record, RecordError>>
  >,
  update: Function as PropType<
    (request: UpdateRecordRequest) => Promise<APIResponse<Record, RecordError>>
  >,
});
...
async function _submit(e: Event) {
  ...
  if (props.create) {
    const createRequest: CreateRecordRequest = {
      ...
      handicap: parseInt(handicap.value),
      komi: parseFloat(komi.value),
  ...
  <form @submit="_submit">
    <div class="m-6 flex">
      <div class="grow">
        <div>
          <input
            v-model="black_player"
            class="px-2 bg-gray-900 text-gray-200 w-full rounded-md"
          />
        </div>
        <ul v-if="fieldErrors.black_player">
          <li
            v-for="error in fieldErrors.black_player"
            :key="error"
      ...
      <div class="grow">
        ...
        <ul v-if="fieldErrors.white_player">
          <li
            v-for="error in fieldErrors.white_player"
            :key="error"
            class="text-sm text-red-600"
          >
            {{ error }}
    ...
    <div class="m-6 flex">
      ...
      <div class="grow">
        ...
        <ul v-if="fieldErrors.handicap">
          <li
            v-for="error in fieldErrors.handicap"
            :key="error"
            class="text-sm text-red-600"
          >
            {{ error }}
    ...
    <div class="m-6 flex">
      ...
      <div class="grow">
        ...
        <ul v-if="fieldErrors.komi">
          <li
            v-for="error in fieldErrors.komi"
            :key="error"
            class="text-sm text-red-600"
          >
            {{ error }}
    ...
    <div class="m-6 flex">
      <button @click="cancel" class="grow m-2 w-full bg-gray-200 rounded-md">
        Cancel
      </button>
      <button type="submit" class="grow m-2 w-full bg-gray-300 rounded-md">
        Done
      </button>
    </div>
  </form>
